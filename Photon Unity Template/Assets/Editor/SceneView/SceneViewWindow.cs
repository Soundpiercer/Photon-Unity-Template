...
public class SceneViewWindow : EditorWindow
{
    ...
    internal void OnGUI()
    {
        ...
        // 에디터 전용.
        GUILayout.Label("Scenes In Editor", EditorStyles.boldLabel);
        for (var i = 0; i < EditorBuildSettings.scenes.Length; i++)
        {
            var scene = EditorBuildSettings.scenes[i];
            if (!scene.enabled)
        ...
        /*
        bool pressedTestBed = GUILayout.Button("TestBed", new GUIStyle(GUI.skin.GetStyle("Button")) { alignment = TextAnchor.MiddleLeft });
        if (pressedTestBed)
        {
            if (EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
